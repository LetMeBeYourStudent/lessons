...
string implode(const vector<string> &elems, const string &delim) {
    string s;
    ...
    if (r.stops.empty())
        ...
        for (const string &stop: r.stops) {
            ...
            if (r.stops_to_bus.at(stop).empty())
                ...
                os << implode(r.stops_to_bus.at(stop), " ");
            if (stop != *(r.stops.end() - 1))
                os << endl;
        }
    }
    return os;
...
ostream &operator<<(ostream &os, const AllBusesResponse &r) {
    ...
    if (r.lines.empty())
        ...
        for (auto ii = buses.begin(); ii != buses.end(); ++ii) {
            os << "Bus " << *(ii) << ": ";
            os << implode(r.lines.at(*(ii)), " ");
            if (ii != (buses.end() - 1))
                os << endl;
        }
    }
...
public:
    ...
    BusesForStopResponse GetBusesForStop(const string &stop) const {
        ...
        for (const auto& bus: buses_)
            if (count(lines_.at(bus).begin(), lines_.at(bus).end(), stop)) result.buses.push_back(bus);
        return result;
    }
